...
def detect_theme(pitanje, teme=TEME):
    ...
    try:
        ...
        print(f"❌ Greška pri detekciji teme (Gemini): {e}")
        return "this query does not match any theme"


def similarity_base_upit(banka_primjera, pitanje):
    print(f"\nProvjeri najslicnije pitanje '{pitanje}' pomoću Gemini modela")
    ...
    try:
        ...
        print(f"❌ Greška pri provjeri sličnosti (Gemini): {e}")
        return None, None
...
def generiraj_sparql(pitanje):
    ...
    try:
        ...
        print(f"❌ Greška pri generiranju SPARQL-a (Gemini): {e}")
        return None


def ispravi_sparql(originalni_upit, poruka_greske, pitanje):
    print("\n🔧 Pokušavam ispraviti upit na temelju greške (Gemini)...")
    ...
    try:
        ...
        print(f"❌ Greška pri ispravljanju SPARQL-a (Gemini): {e}")
        return None


def provjeri_logiku_upita(originalni_upit, pitanje):
    print(
        f"\n🔍 PROVJERA LOGIKE (Gemini): Analiziram upit za pitanje: '{pitanje}'"
    )
    ...
    try:
        ...
        if "ISPRAVNO" in ai_odgovor.upper() and "NE" not in ai_odgovor.upper():
            ...
            response_ispravak = client.models.generate_content(
                model=MODEL_NAME,
                contents=prompt_za_ispravak,
                config={
                    "temperature": 0.0,
                    "thinking_config":{
                        "thinking_level": "minimal"
            ...
            ispravljeni_upit = response_ispravak.text
        ...
        print(f"❌ Greška pri provjeri logike (Gemini): {e}")
        return None