...
def _pozovi_openai(prompt):
    """Pomoćna funkcija za lakše slanje poruka OpenAI API-ju."""
...
def detect_theme(pitanje, teme=TEME):
    ...
    try:
        odgovor = _pozovi_openai(detailed_prompt).strip()
        print(f"\n tema pitanja: '{odgovor}'")
        return odgovor

    except Exception as e:
        print(f"❌ Greška pri detekciji teme (ChatGPT): {e}")
        return "this query does not match any theme"


def similarity_base_upit(banka_primjera, pitanje):
    print(f"\nProvjeri najslicnije pitanje '{pitanje}' pomoću ChatGPT modela")
    detailed_prompt = SIMILARITY_BASE_ONE_SHOT_PROMPT_TEMPLATE.format(
        banka_primjera=banka_primjera, pitanje=pitanje
    )
    try:
        ai_odgovor = _pozovi_openai(detailed_prompt)
        print(f"\n AI odabir najsličnijeg pitanja: '{ai_odgovor}'")
        for entry in BANKA_PRIMJERA:
            if entry["pitanje"] in ai_odgovor:
                return entry["sparql"], entry["pitanje"]
        return None, None
    except Exception as e:
        print(f"❌ Greška pri provjeri sličnosti (ChatGPT): {e}")
        return None, None
...
def generiraj_sparql(pitanje):
    ...
    try:
        ai_odgovor = _pozovi_openai(final_prompt)
        ...
        print(f"❌ Greška pri generiranju SPARQL-a (ChatGPT): {e}")
        return None


def ispravi_sparql(originalni_upit, poruka_greske, pitanje):
    print("\n🔧 Pokušavam ispraviti upit na temelju greške (ChatGPT)...")
    ...
    try:
        ai_odgovor = _pozovi_openai(prompt_ispravka)
        if "```sparql" in ai_odgovor:
            sparql_kod = ai_odgovor.split("```sparql\n")[1].split("```")[0]
        else:
            sparql_kod = ai_odgovor
        ...
        print(f"❌ Greška pri ispravljanju SPARQL-a (ChatGPT): {e}")
        return None


def provjeri_logiku_upita(originalni_upit, pitanje):
    print(
        f"\n🔍 PROVJERA LOGIKE (ChatGPT): Analiziram upit za pitanje: '{pitanje}'"
    )
    ...
    try:
        ...
        if "ISPRAVNO" in ai_odgovor.upper() and "NE" not in ai_odgovor.upper():
            ...
            ispravljeni_upit = _pozovi_openai(prompt_za_ispravak)
        ...
        print(f"❌ Greška pri provjeri logike (ChatGPT): {e}")
        return None